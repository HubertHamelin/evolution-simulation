...
public class BlrSensor extends Sensor {

    Point location;
    Direction direction;

    public BlrSensor(Point location, Direction direction) {
        this.name = "Blr";
        this.value = 0.0;
    ...
    public void update(int[][] environment, int time) {
        ...
        if (this.direction == Direction.NORTH) {
            if (this.location.x + 1 >= environment[0].length || environment[this.location.y][this.location.x + 1] != 0)
                this.value = 1;
            else if (this.location.x - 1 < 0 || environment[this.location.y][this.location.x - 1] != 0)
                this.value = 1;
            else
                this.value = 0;
            ...
            if (this.location.y + 1 >= environment.length || environment[this.location.y + 1][this.location.x] != 0)
                this.value = 1;
            else if (this.location.y - 1 < 0 || environment[this.location.y - 1][this.location.x] != 0)
                this.value = 1;
            else
                this.value = 0;
